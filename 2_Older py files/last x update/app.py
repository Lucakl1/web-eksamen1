...
def home():
    try:
        ...
        return render_template("home.html", lan=lan, tweets=tweets, trends=trends, suggestions=suggestions, user=user)
    except Exception as ex:
        ic(ex)
        return "error"
    finally:
...
def profile():
    try:
        ...
        cursor.execute(q, (user["user_pk"],))
        user = cursor.fetchone()
        profile_html = render_template("_profile.html", lan=lan)
...
def api_create_post():
    try:
        user = session.get("user", "")
        if not user: return x.lans("invalid_user")
        ...
        if "x-error post" in str(ex):
            toast_error = render_template("___toast_error.html", message=f"Post - {x.POST_MIN_LEN} x.lans('to') {x.POST_MAX_LEN} x.lans'characters'")
